...
function DisplayLectures() {
  ...
  const { lectures, accessDenied } = useSelector((state) => state.lecture);
  const { role } = useSelector((state) => state.auth);
  ...
  async function onLectureDelete(courseId, lectureId) {
    if (window.confirm('Are you sure you want to delete this lecture?')) {
      toast('Deleting lecture...');
      await dispatch(
        deleteCourseLecture({ courseId: courseId, lectureId: lectureId })
      );
      await dispatch(getCourseLectures(courseId)); // Refetch lectures
    }
  }
  ...
  return (
    <Layout>
      <div className="flex flex-col gap-10 items-center justify-center min-h-[90vh] py-10 text-white mx-4 sm:mx-0">
        <h1 className="text-2xl font-semibold text-center text-yellow-500">
          Course: {state?.title}
        </h1>

        {lectures && lectures.length > 0 ? (
          <div className="flex flex-col md:flex-row justify-center gap-10 w-full max-w-6xl">
            {/* Left side: video player and description */}
            <div className="space-y-5 w-full md:w-8/12 p-4 rounded-lg shadow-[0_0_10px_black]">
              <video
            ...
            <ul className="w-full md:w-4/12 p-4 rounded-lg shadow-[0_0_10px_black] space-y-4 max-h-[70vh] overflow-y-auto">
              <li className="font-semibold text-xl text-yellow-500 flex items-center justify-between">
                <p>Lectures list</p>
                {role === 'ADMIN' && (
                  <button
              ...
              {lectures.map((lecture, idx) => (
                <li
                  className={`space-y-2 p-2 rounded-md ${
                    currentVideo === idx ? 'bg-gray-700' : 'bg-gray-900'
                  } cursor-pointer`}
                  key={lecture._id}
                >
                  <p
          ...
          !accessDenied && (
            <h2 className="text-xl font-semibold">
              {role === 'ADMIN'
                ...
                : 'Lectures are not yet available for this course.'}
            </h2>
          )
        )}
      </div>
